<template>
  <section class="overflow-hidden w-screen h-screen flex flex-col lg:flex-row lg:items-center justify-end">
    ...
    <div
      class="flex pl-5 pr-5 flex-col gap-3 lg:pt-0 lg:gap-5 lg:w-3/4 lg:h-4/6 lg:pl-20"
    >
      <!-- Título -->
      <h1 class="text-3 lg:text-6">{{ title }}</h1>

      <!-- Texto Largo -->
      <p class="text-s3 lg:w-[90%] lg:text-p4">{{ content }}</p>
...
export default {
  props: {
    ...
    buttonClass: {
      type: String,
      default:
        " bg-oxford-blue text-columbia-blue hover:bg-black hover:text-white transition duration-200 flex justify-between p-3 pr-3 pl-3 w-28 text-s2 lg:w-80 lg:p-6 lg:text-p3",
    },
    imageSrc: {
      type: String,
      required: true,
